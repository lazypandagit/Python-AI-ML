...
# oppening files
def fileOpen(
    main: str = MAIN_FILE_NAME, temp: str = TEMP_FILE_NAME
) -> tuple[BufferedReader, BufferedWriter]:
    return open(main, "rb"), open(temp, "ab")
...
# function to see if aadhar number is unique or not
def uniqeornot(aadhar: int, file: BufferedWriter) -> bool:
    try:
        while True:
            data: dict[str, int | str | list[str]] = pickle.load(file)
            if data["Aadhar Number"] == aadhar:
                return False
            else:
                pass
    except EOFError:
...
def insert(file: BufferedWriter) -> None:
    ...
    while u == False:
        if uniqeornot(aadhar, file) != True:
            print("The Aadhar Number already exist in records")
            aadhar: int = getAadhar()
        else:
            break
    age: int = getAge()
...
def find(file: BufferedReader, aadhar: int) -> bool:
    try:
        while True:
            data: dict[str, int | str | list[str]] = pickle.load(file)
            if data["Aadhar Number"] == aadhar:
...
def update() -> None:
    ...
    try:
        while True:
            # opening files
            mainfile, tempfile = fileOpen()
            recordPresent = find(mainfile, aadhar)
            ...
            if recordPresent == False:
                ...
                # if record found
                print("Record Found:")
                data = copyrecords(mainfile, tempfile, aadhar)
                printRecord(data)
                stop()
                while True:
                    if mainfile.closed == True:
                        mainfile, tempfile = fileOpen()
                    currentUpdate = 0
                    clear()
                    data = copyrecords(mainfile, tempfile, aadhar)
                    ...
                    if c == "1":
                        ...
                        # updating vaccines
                        vcnData = getVaccineTypes("Enter Updated Vaccines: ")
                        newData: dict[str, int | str | list[str]] = {
                            "Aadhar Number": aadhar,
                            "Name": data["Name"],
                            "Age": data["Age"],
                            "Vaccine type": vcnData,
                        }
                        print("'Vaccine'parameter updated")
                        pickle.dump(newData, tempfile)
                        data["Vaccine type"] = vcnData
                        updates += 1
                        currentUpdate += 1
                        fileClose(mainfile, tempfile)
                        replaceFile(MAIN_FILE_NAME, TEMP_FILE_NAME)
